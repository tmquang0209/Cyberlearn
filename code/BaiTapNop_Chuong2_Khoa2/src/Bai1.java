...
/*
 * Tên chương trình: tìm số chẵn đầu tiên trong mảng
 * Ngày: 24/08/2021
 */
...
public class Bai1 {
	...
	public static void timSoChanDauTien(int arr[][], int row, int col) {
		boolean flag;
		for (int i = 0; i < row; i++) {
			flag = false;
			for (int j = 0; j < col; j++) {
				if (arr[i][j] % 2 == 0) {
					flag = true;
					System.out.println("So chan dau tien trong mang la: " + arr[i][j]);
					break;
				}
			}
			if (flag)
