/*
 * Tên chương trình: xuất ra màn hình số xuất hiện nhiều nhất/ít nhất
 * Ngày tạo: 21/08/2021
 */
...
public class Bai16 {
	...
	public static int soXuatHienItNhat(int mang[]) {
		int mangDem[] = demPhanTu(mang);
		int min = mangDem[0];
		for (int i = 1; i < mang.length; i++) {
			if (mangDem[i] != 0 && mangDem[i] < min) {
				min = mangDem[i];
			}
		}
		return min;
	...
	public static void inRaManHinh(int mang[]) {
		...
		for (int i = 0; i < mang.length; i++) {
			if (mangDem[i] != 0 && mangDem[i] == soXuatHienNhieuNhat(mang)) {
				System.out.println("Số xuất hiện nhiều nhất: " + mang[i] + "[" + mangDem[i] + " lần] \t");
				break;
			}
		}
		for (int i = 0; i < mang.length; i++) {
			if (mangDem[i] != 0 && mangDem[i] == soXuatHienItNhat(mang)) {
				System.out.println("Số xuất hiện ít nhất: " + mang[i] + "[" + mangDem[i] + " lần] \t");
				break;
			}
		}
	}
